...
RSpec.describe 'message show page' do
  context "#happypath" do 
    ...
    it 'displays message content' do
      ...
      expect(page).to have_content("What a piece of work is man! How noble in reason, how infinite in faculty! In form and moving how express and admirable! In action how like an angel, in apprehension how like a god! The beauty of the world. The paragon of animals.")
    end
  ...
  context "#sadpath" do 
    before :each do
        ...
        allow(SatelliteService).to receive(:get_norad_id).and_return(@sat_db_call)
        allow(SatelliteService).to receive(:get_satellite_position).and_return(@sat_position_call)
        allow(MessageService).to receive(:get_message).and_return(404)

        visit '/auth/google_oauth2'
    end

    it 'displays error message content' do
      visit "/messages/999" 
      
      expect(page).to have_content("You broke the space-time continuum and your message disappeared into the void")
    end
  end 
end